...
#'
#' This function fits a Zero-Inflated INAR(1) (ZI-INAR(1)) or Hurdle INAR(1) (H-INAR(1)) model
#' using Stan and returns the model fit.
#'
#' @param mod_type Character string indicating the model type. Use "zi" for zero-inflated
#' models and "h" for hurdle models.
#' @param distri Character string specifying the distribution. Options are "poi" for Poisson
#' or "nb" for Negative Binomial.
#' @param y A numeric vector of integers representing the observed data.
#' @param n_pred Integer specifying the number of time points for future predictions (default is 4).
#' @param thin Integer indicating the thinning interval for Stan sampling (default is 2).
#' @param chains Integer specifying the number of Markov chains to run (default is 1).
#' @param iter Integer specifying the total number of iterations per chain (default is 2000).
#' @param warmup Integer specifying the number of warmup iterations per chain (default is \code{iter/2}).
